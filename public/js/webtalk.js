(function($, io) {
  ...
  function setSize(box, classClosed, classSelf, classBoth) {
    return function(thisOpen, otherOpen) {
      ...
      if (!thisOpen) {
        // This is closed
        box.removeClass(classSelf + ' ' + classBoth).addClass(classClosed);
      }
      else {
        // This is open
        if (otherOpen) {
          // Other is also open
          box.removeClass(classSelf + ' ' + classClosed).addClass(classBoth);
        }
        else {
          // Other is closed
          box.removeClass(classBoth + ' ' + classClosed).addClass(classSelf);
        }
      }
    };
  }
  ...
  //         if (!users[i].available) user$.addClass("user-unavailable");
  ...
  //     msgNew(messageModel.types.CALL, 'Calling <strong>' + username + '</strong>...', '',
  //     null,
  ...
  //   if (messageModel.canReplace()) {
  //     msgNew(messageModel.types.RING, '<strong>' + username + '</strong> wants to chat!', '',
  ...
  function errorHandler() {
    msgNew(messageModel.types.ERR, "Authentication Error", "Please sign in before chatting.",
    new Btn("OK", function() {
